<script>
import appFlagsMovies from './appFlagsMovies.vue'
import appFlagsSeries from './appFlagsSeries.vue'
import { store } from '../store.js'
export default {
    components:{
        appFlagsMovies,
        appFlagsSeries
    },
    data(){
        return{
            store,
        }
    },
    computed:{
        starsMovies(){
            let votesArray = []
            store.moviesArray.filter((elem) =>{
                let starsArray = Math.floor(elem.vote_average / 2)
                return votesArray.push(starsArray)
            })
            return votesArray
        },
        starsSeries(){
            let votesArray = []
            store.seriesArray.filter((elem) =>{
                let starsArray = Math.floor(elem.vote_average / 2)
                return votesArray.push(starsArray)
            })
            return votesArray
        }
    }
}
</script>
<template lang="">
    <div class="container">
        <div class="row">
            <div>
                <ul>
                    <li v-for="(movie, index) in store.moviesArray" :key="index">
                        <div> 
                            <img :src="store.imagesArrayMovies[index]">
                        </div>
                        <appFlagsMovies :moviesId="movie"/>
                        <div v-if="starsMovies[index] !== 0">
                            <i v-for="(star, index) in starsMovies[index]" :key="index" class="fa-solid fa-star"></i>
                        </div>
                        <div v-else>NO STAR</div>
                    </li>
                </ul>
            </div>
            <div>
                <ul>
                    <li v-for="(serie, index) in store.seriesArray" :key="index">
                        <div v-if="serie.poster_path !== null"> 
                            <img :src="store.imagesArraySeries[index]">
                        </div>
                        <div class="not_available" v-else>
                            <img src="data:image/png;base64," alt="">
                        </div>
                        <appFlagsSeries :seriesId="serie"/>
                        <div v-if="starsSeries[index] !== 0">
                            <i v-for="(star, index) in starsSeries[index]" :key="index" class="fa-solid fa-star"></i>
                        </div>
                        <div v-else>NO STAR</div>
                    </li>
                </ul>
            </div>
        </div>
    </div>
</template>
<style lang="scss" scoped>
    .container{
        .row{
            display: flex;
            justify-content: space-around;
            .not_available{
                img{
                    width: 342px;
                }
            }
        }
    }
</style>